...
```{r}
cran3 <- cran3 %>%mutate(size_gb = size / (2^30))
```
...
```{r}
cran3 %>% summarise(avg_bytes = mean(size, na.rm = TRUE))
```
# Agrupamientos
...
```{r}
by_package%>% summarise(avg_size  = mean(size, na.rm=T))
```
...
```{r}
pack_sum <- by_package %>%summarise(count = n(),unique = n_distinct(ip_id),countries = n_distinct(country),med_bytes = stats::median(size, na.rm = TRUE))
...
```{r}
cran %>% group_by(package) %>%summarise(count = n()) %>%filter(count > quantile(count, probs = 0.99))
